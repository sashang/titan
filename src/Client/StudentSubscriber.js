...
export default class StudentSubscriber extends Component {
  ...
  createSubscriber() {
    ...
    console.log(`Subscribing to: ${this.state.stream.name}`);
    this.subscriberId = uuid();
    const { subscriberId } = this;
    if (this.state.stream.name == 'mary.poppins.tewtin@gmail.com')
        return;
    const subscriber = this.state.session.subscribe(
      ...
      (err) => {
        if (subscriberId !== this.subscriberId || this.state.stream.name) {
...
StudentSubscriber.defaultProps = {
  stream: null,
  session: null,
  properties: {},
  eventHandlers: null,
...
StudentSubscriber.contextTypes = {
  stream: PropTypes.shape({
    streamId: PropTypes.string,
  }),
  session: PropTypes.shape({
